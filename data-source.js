...
function calculateLiquidacion(operation, input = {}) {
  ...
  const comisionFacturadoProdPct = parseMoney(source.comisionFacturadoProdPct !== undefined ? source.comisionFacturadoProdPct : source.comisionProd);
  const comisionFacturadoCompPct = parseMoney(source.comisionFacturadoCompPct !== undefined ? source.comisionFacturadoCompPct : source.comisionComp);
  const comisionEfectivoProdPct = parseMoney(source.comisionEfectivoProdPct);
  const comisionEfectivoCompPct = parseMoney(source.comisionEfectivoCompPct);
  const hasInput = (key) => Object.prototype.hasOwnProperty.call(input, key);
  const comisionFacturadoProd = hasInput("comisionFacturadoProd") ? parseMoney(input.comisionFacturadoProd) : facturado * comisionFacturadoProdPct / 100;
  const comisionFacturadoComp = hasInput("comisionFacturadoComp") ? parseMoney(input.comisionFacturadoComp) : facturado * comisionFacturadoCompPct / 100;
  const efectivoConIvaProd = efectivoProd + (frigo ? efectivoProd * 0.105 : 0);
  const comisionEfectivoProd = hasInput("comisionEfectivoProd") ? parseMoney(input.comisionEfectivoProd) : efectivoConIvaProd * comisionEfectivoProdPct / 100;
  const comisionEfectivoComp = hasInput("comisionEfectivoComp") ? parseMoney(input.comisionEfectivoComp) : efectivoComp * comisionEfectivoCompPct / 100;
